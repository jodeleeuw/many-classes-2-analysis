...
secondary_model_data <- student_level_model_data %>%
  left_join(submission_level_model_data %>% group_by(participant_id) %>% slice_head(n=1) %>% select(participant_id, course_id, initiate_playback, submDaysBeforeDueDate) %>% ungroup(), by=join_by(participant_id, course_id))
```
