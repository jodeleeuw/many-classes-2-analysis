...
ggplot(moderator_forest_plot_data, aes(x=Estimate, y=effect, xmin=Q2.5, xmax=Q97.5))+
  geom_pointrange()+
  facet_grid(model_level~., scales="free_y", space="free")+
  theme_bw()+
  theme(panel.grid.major.y=element_blank())+
...
```{r}
summary(did_view_prequestion_score_fit)
```
...
```{r}
ggplot(secondary_model_data, aes(x=prequestion_score, y=initiate_playback, color=condition))+
...
- [ ] Visualize the pre question score student level moderator
- [ ] Remove the overall model, since we aren't using it anywhere. 
